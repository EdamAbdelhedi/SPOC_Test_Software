#!/usr/bin/env python3
"""End-to-end CUT + MUX + BNC frequency check built on the reusable classes."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Route one CUT PWM signal through MUX to the BNC and measure frequency"
    )
    ...
    parser.add_argument(
        "--all-pwm-pins",
        default="false",
        help="When true, run the frequency test on every built-in PWM-capable CUT pin",
    )
    parser.add_argument("--channel", default="ch1", help="Shield channel name, for example ch1")
    parser.add_argument("--freq-values", nargs="+", type=int, default=[1000], help="Frequency values in Hz")
...
def measure_frequency_step(
    ...
    *,
    ...
    command = cut.set_pwm(pin=cut_pin, freq_hz=expected_freq_hz, duty=duty)
    applied_freq_hz = command.freq_hz
    prepare_scope(scope, scope_channel, applied_freq_hz, trigger_edge_level, timebase_scale)
    ...
    if measured_freq_hz is None:
        print("frequency measurement failed: the oscilloscope could not extract a valid frequency")
        return False

    err_pct = pct_error(measured_freq_hz, applied_freq_hz)
    passed = err_pct <= freq_tol_pct
    print(
        f"channel={channel} scope_channel={scope_channel} cut_pin={cut_pin} "
        f"requested_freq={expected_freq_hz}Hz expected_freq={applied_freq_hz}Hz "
        f"measured_freq={measured_freq_hz:.2f}Hz error={err_pct:.2f}% tolerance={freq_tol_pct:.2f}% "
        f"result={'PASS' if passed else 'FAIL'}"
    )
    return passed


def run_frequency_case(
    scope: Oscilloscope,
    mux: MuxController,
    cut: CutPwmController,
    *,
    ...
    prepare_scope(scope, scope_channel, resolve_pwm_frequency_hz(freq_values[0]), trigger_edge_level, timebase_scale)
    print(
        f"Initial state: preparing one PWM route on {cut_pin} and {channel}. "
        f"Using Rigol CH{scope_channel} for {channel} with PROBe x{SCOPE_CHANNEL_PROBE} "
        f"and CHANnel:SCALe {SCOPE_CHANNEL_SCALE} V/div."
    )
    mux.route(channel, cut_pin)
    print_route_diagnostics(cut, mux, channel, cut_pin)

    total_steps = len(freq_values)
    for step, freq_hz in enumerate(freq_values, start=1):
        ...
        print(
            ...
            f"with requested duty={duty * 100:.1f}% applied duty={applied_duty * 100:.1f}%"
        )
        if measure_frequency_step(
            scope,
            cut,
            channel=channel,
            cut_pin=cut_pin,
...
def main() -> int:
    ...
    try:
        ...
        for freq_hz in args.freq_values:
    ...
    try:
        ...
        for index, cut_pin in enumerate(pin_plan, start=1):
            print(f"\n=== Frequency case {index}/{len(pin_plan)}: {cut_pin} ===")
            case_pass_count = 0
            case_fail_count = 0
            try:
                case_pass_count, case_fail_count = run_frequency_case(
                    scope,
                    mux,
                    cut,
                    channel=channel,
                ...
                try:
                    cut.disable(pin=cut_pin, freq_hz=args.freq_values[-1])
                except Exception:
                    pass
                try:
        ...
        total_steps = len(pin_plan) * len(args.freq_values)
        print(f"\nSummary: {pass_count} frequency PASS, {fail_count} frequency FAIL, total {total_steps}")
